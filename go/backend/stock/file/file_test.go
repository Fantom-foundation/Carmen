...
func TestFile_CanParticipateInTwoPhaseCommit(t *testing.T) {
	commitDir := t.TempDir()
	stockDir := t.TempDir()

	stock, err := openStock[int, int](stock.IntEncoder{}, stockDir)
	if err != nil {
...
func TestFile_CanBeCommittedAndSealed(t *testing.T) {
	stock, err := openStock[int, int](stock.IntEncoder{}, t.TempDir())
	if err != nil {
	...
	if want, got := utils.TwoPhaseCommit(1), stock.lastCommit; want != got {
...
func TestFile_CanBeRolledBackInTwoPhaseCommit(t *testing.T) {
	stock, err := openStock[int, int](stock.IntEncoder{}, t.TempDir())
	if err != nil {
		t.Errorf("failed to open stock: %v", err)
	}
	...
	if want, got := utils.TwoPhaseCommit(0), stock.lastCommit; want != got {
		t.Errorf("unexpected last commit: want %d, got %d", want, got)
	}
...
func TestFile_CommitStateIsPersisted(t *testing.T) {
	dir := t.TempDir()
	encoder := stock.IntEncoder{}
...
func TestFile_NumberOfCommitValuesIsPersisted(t *testing.T) {
	dir := t.TempDir()
	encoder := stock.IntEncoder{}
	stock, err := openStock[int, int](encoder, dir)
	if err != nil {
...
func TestFile_CheckCanRecoverFromCrashAfterPrepare(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			dir := t.TempDir()
			first, err := openStock[int, int](stock.IntEncoder{}, dir)
			if err != nil {
				t.Fatalf("failed to open stock: %v", err)
			}
			...
			second, err := openStock[int, int](stock.IntEncoder{}, dir)
			if err != nil {
				t.Fatalf("failed to open stock: %v", err)
			}

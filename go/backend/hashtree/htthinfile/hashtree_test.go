...
func TestHashtreeInitialState(t *testing.T) {
	...
	pages := [][]byte{}
	tree := CreateHashTreeFactory(tmpDir+"/hashesfile", 3).Create(testingPageProvider{pages: pages})
...
func TestTwoTreesWithSameStateProvidesSameHash(t *testing.T) {
	...
	treeA := CreateHashTreeFactory(tmpDirA+"/hashesfile", 3).Create(testingPageProvider{pages: pagesA})
	treeB := CreateHashTreeFactory(tmpDirB+"/hashesfile", 3).Create(testingPageProvider{pages: pagesB})
	for i := 0; i < 8; i++ {
		treeA.MarkUpdated(i)
		treeB.MarkUpdated(i)
	}

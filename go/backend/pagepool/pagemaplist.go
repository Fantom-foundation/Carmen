...
func NewPageMapList[K comparable, V comparable](bucket, pageItems int, pagePool *PagePool[K, V]) *PageMapList[K, V] {
...
func InitPageMapList[K comparable, V comparable](bucket, pageItems int, pagePool *PagePool[K, V], data []common.MapEntry[K, V]) (*PageMapList[K, V], error) {
	delegate := NewPageList[K, V](bucket, pageItems, pagePool)
	p := &PageMapList[K, V]{
		delegate: delegate,
	}

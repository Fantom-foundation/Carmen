...
func NewPageList[K comparable, V comparable](bucket, pageItems int, pagePool *PagePool[*KVPage[K, V]]) *PageList[K, V] {
	return &PageList[K, V]{
...
func (m *PageList[K, V]) createNextPage(page *KVPage[K, V]) (nextId PageId) {
	tailPageId := m.pagePool.GenerateNextId()
	m.pageList = append(m.pageList, tailPageId)
	page.setNext(tailPageId)
	return NewPageId(m.bucket, tailPageId)
}
...
func fillFromTail[K comparable, V comparable](page, tail *KVPage[K, V], number int) int {
	n := number
	if number > tail.size() {
		n = tail.size()
	}
...
func (m *PageList[K, V]) next(current *KVPage[K, V]) (page *KVPage[K, V], err error) {
	if current.hasNext {

...
func TestStateDB_StateDBCanNotEndABlockIfCommitIsNotAllowed(t *testing.T) {
	ctrl := gomock.NewController(t)
	mock := database.NewMockState(ctrl)
	db := createStateDBWith(mock, nil, 1024)
...
func TestStateDB_CollectsErrorsAndReportsThemDuringACheck(t *testing.T) {
	...
	tests := map[string]struct {
		setExpectations func(state *database.MockDatabase)
		applyOperation  func(db State)
	}{
		"exits": {
			setExpectations: func(db *database.MockDatabase) {
				db.EXPECT().Exists(gomock.Any()).Return(false, injectedError)
			},
			applyOperation: func(db State) {
				db.Exist(address1)
			},
		...
		"balance": {
			setExpectations: func(db *database.MockDatabase) {
				db.EXPECT().GetBalance(gomock.Any()).Return(common.Balance{}, injectedError)
			},
			applyOperation: func(db State) {
				db.GetBalance(address1)
			},
		...
		"nonce": {
			setExpectations: func(db *database.MockDatabase) {
				db.EXPECT().GetNonce(gomock.Any()).Return(common.Nonce{}, injectedError)
			},
			applyOperation: func(db State) {
				db.GetNonce(address1)
			},
		...
		"code": {
			setExpectations: func(db *database.MockDatabase) {
				db.EXPECT().GetCode(gomock.Any()).Return([]byte{}, injectedError)
			},
			applyOperation: func(db State) {
				db.GetCode(address1)
			},
		...
		"code-hash": {
			setExpectations: func(db *database.MockDatabase) {
				db.EXPECT().Exists(address1).Return(true, nil)
				db.EXPECT().GetCodeHash(gomock.Any()).Return(common.Hash{}, injectedError)
			},
			applyOperation: func(db State) {
		...
		"code-size": {
			setExpectations: func(db *database.MockDatabase) {
				db.EXPECT().GetCodeSize(gomock.Any()).Return(0, injectedError)
			},
			applyOperation: func(db State) {
		...
		"storage": {
			setExpectations: func(db *database.MockDatabase) {
				db.EXPECT().GetStorage(gomock.Any(), gomock.Any()).Return(common.Value{}, injectedError)
			},
			applyOperation: func(db State) {
				db.GetState(address1, key1)
			},
		...
		"apply": {
			setExpectations: func(db *database.MockDatabase) {
				db.EXPECT().Exists(address1).Return(true, nil)
				db.EXPECT().Apply(gomock.Any(), gomock.Any()).Return(injectedError)
			},
			applyOperation: func(db State) {
				db.SetNonce(address1, 12)
				db.EndBlock(2)
		...
		"get-hash": {
			setExpectations: func(db *database.MockDatabase) {
				db.EXPECT().GetHash().Return(common.Hash{}, injectedError)
			},
			applyOperation: func(db State) {
				db.GetHash()
			},
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			...
			state := CreateStateDBUsing(db)

			if err := state.Check(); err != nil {
				t.Errorf("unexpected error at begin of test: %v", err)
			}

			test.setExpectations(db)
			test.applyOperation(state)
			if err := state.Check(); !errors.Is(err, injectedError) {
...
func TestStateDB_CanCollectMoreThanOneError(t *testing.T) {
	ctrl := gomock.NewController(t)
	state := database.NewMockDatabase(ctrl)
...
func TestStateDB_GetArchiveStateDbCanProduceArchiveAccess(t *testing.T) {
	ctrl := gomock.NewController(t)
	state := database.NewMockDatabase(ctrl)
	archive := database.NewMockDatabase(ctrl)
	db := CreateStateDBUsing(state)
...
func TestStateDB_GetArchiveStateDbRecyclesNonCommittableStateDbInstances(t *testing.T) {
	ctrl := gomock.NewController(t)
	state := database.NewMockDatabase(ctrl)
	archive := database.NewMockState(ctrl)
	db := CreateStateDBUsing(state)
...
func TestStateDB_GetArchiveBlockHeightReturnsHeightOfArchive(t *testing.T) {
	tests := map[string]struct {
		...
		"missing":   {0, false, database.NoArchiveError},
	}

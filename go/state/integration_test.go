...
func getAllDatabaseConfigurations() []namedDatabaseConfig {
...
func TestIntegration_CanHandleMaximumBalance(t *testing.T) {
	...
	for _, config := range getAllDatabaseConfigurations() {
		config := config
		t.Run(config.name(), func(t *testing.T) {
			t.Parallel()
			dir := t.TempDir()
			store, err := config.createDatabase(dir)
			if err != nil {
...
func TestIntegration_ThereCanBeMultipleBulkLoadPhasesOnRealState(t *testing.T) {
	for _, config := range getAllDatabaseConfigurations() {
		config := config
		t.Run(config.name(), func(t *testing.T) {
			t.Parallel()
...
func TestIntegration_BulkLoadsCanBeInterleavedWithRegularUpdates(t *testing.T) {
	for _, config := range getAllDatabaseConfigurations() {
		config := config
		t.Run(config.name(), func(t *testing.T) {
			t.Parallel()
...
func testIntegration_StateDbHashAfterModification(t *testing.T, mod func(s state.State)) {
	want := map[database.Schema]common.Hash{}

	allSchemas := map[database.Schema]bool{}
	for _, config := range getAllDatabaseConfigurations() {
		allSchemas[config.config.Schema] = true
	}

	for _, s := range maps.Keys(allSchemas) {
		// The reference version for hash checks is the go-in-memory variant.
		referenceImpl, err := database.NewDatabase(database.Parameters{
			Directory: t.TempDir(),
			Schema:    s,
		...
		ref := state.CreateStateDBUsing(referenceImpl)
		defer ref.Close()
		mod(ref)
		ref.EndTransaction()
		ref.EndBlock(1)
	...
	for i := 0; i < 3; i++ {
		for _, config := range getAllDatabaseConfigurations() {
			...
			t.Run(fmt.Sprintf("%v/run=%d", config.name(), i), func(t *testing.T) {
				t.Parallel()
				store, err := config.createDatabase(t.TempDir())
				if err != nil {
...
func TestIntegration_StateHashIsDeterministicForEmptyState(t *testing.T) {
	testIntegration_StateDbHashAfterModification(t, func(s state.State) {
		// nothing
	})
}

func TestIntegration_StateHashIsDeterministicForSingleUpdate(t *testing.T) {
	testIntegration_StateDbHashAfterModification(t, func(s state.State) {
		s.SetState(address1, key1, val1)
	})
}

func TestIntegration_StateHashIsDeterministicForMultipleUpdate(t *testing.T) {
	testIntegration_StateDbHashAfterModification(t, func(s state.State) {
		s.SetState(address1, key1, val1)
...
func TestIntegration_StateHashIsDeterministicForMultipleAccountCreations(t *testing.T) {
	testIntegration_StateDbHashAfterModification(t, func(s state.State) {
...
func TestIntegration_StateHashIsDeterministicForMultipleAccountModifications(t *testing.T) {
	testIntegration_StateDbHashAfterModification(t, func(s state.State) {
		s.CreateAccount(address1)
		s.CreateAccount(address2)
		s.CreateAccount(address3)
...
func TestIntegration_StateHashIsDeterministicForMultipleBalanceUpdates(t *testing.T) {
	testIntegration_StateDbHashAfterModification(t, func(s state.State) {
...
func TestIntegration_StateHashIsDeterministicForMultipleNonceUpdates(t *testing.T) {
	testIntegration_StateDbHashAfterModification(t, func(s state.State) {
...
func TestIntegration_StateHashIsDeterministicForMultipleCodeUpdates(t *testing.T) {
	testIntegration_StateDbHashAfterModification(t, func(s state.State) {
...
// TestIntegration_PersistentStateDB modifies stateDB first, then it is closed
// and is re-opened in another process, and it is tested that data are available,
// i.e. all was successfully persisted
func TestIntegration_PersistentStateDB(t *testing.T) {
	for _, config := range getAllDatabaseConfigurations() {
		// skip in-memory
		...
		t.Run(config.name(), func(t *testing.T) {
			...
			common.ExecuteTestInSubProcess(t, "TestIntegration_Read", "-db-dir="+dir, "-db-impl="+config.name())
		})
	}
}
...
func createDatabase(t *testing.T, name, dir string) database.Database {
	for _, config := range getAllDatabaseConfigurations() {
		if config.name() == name {
			db, err := config.createDatabase(dir)
			if err != nil {
...
// TestIntegration_Read verifies data are available in a stateDB.
...
func TestIntegration_SupportsConcurrentAccesses(t *testing.T) {
		...
		t.Run(config.name(), func(t *testing.T) {
			...
			for i := 0; i < N; i++ {
				...
				go func() {
					...
					for j := 0; j < M; j++ {
						if isPrimary {
							stateDb.(state.State).BeginBlock()
						}
						stateDb.BeginTransaction()
						...
						stateDb.EndTransaction()
						if isPrimary {
							stateDb.(state.State).EndBlock(uint64(block))
							block++
						} else {
							stateDb.(state.NonCommittableState).Release()
							stateDb = state.CreateNonCommittableStateDBUsing(s)
						}
					}
				}()

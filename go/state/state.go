...
// VmState provides a transaction processing context for smart contracts
// executed by an EVM implementation.
...
// State serves as the public interface definition of a Carmen database. In addition
// to the VmState interface offered for the execution of transactions, operational
// tasks like block commitment, error checking, flushing and closing, and archive
// query operations are supported.
type State interface {
	...
	GetArchiveStateDB(block uint64) (NonCommittableState, error)
...
// NonCommittableState is the public interface offered for views on states that can not
...
type NonCommittableState interface {
	VmState
...
type stateDB struct {
	// The underlying state to read state to.
	state database.State

	// An optional underlying database to write changes to. This is nil for non-committable instances.
...
func CreateStateDBUsing(db database.Database) State {
	return CreateCustomStateDBUsing(db, defaultStoredDataCacheSize)
}
...
func CreateCustomStateDBUsing(db database.Database, storedDataCacheSize int) State {
	if storedDataCacheSize <= 0 {
		storedDataCacheSize = defaultStoredDataCacheSize
	}
	return createStateDBWith(db, db, storedDataCacheSize)
}
...
// Note: any StateDB instanced becomes invalid if the underlying state is
// modified by any other StateDB instance or through any other direct modification.
func CreateNonCommittableStateDBUsing(db database.State) NonCommittableState {
	...
	state := nonCommittableStateDbPool.Get().(*stateDB)
	state.resetState(db)
	return &nonCommittableStateDB{state}
}

func createStateDBWith(state database.State, db database.Database, storedDataCacheCapacity int) *stateDB {
	return &stateDB{
		state:             state,
		database:          db,
		accounts:          map[common.Address]*accountState{},
...
func (s *stateDB) EndBlock(block uint64) {
	...
	if err := s.database.Apply(block, update); err != nil {
...
func (s *stateDB) StartBulkLoad(block uint64) BulkLoad {
	...
	return &bulkLoad{s.database, common.Update{}, block, nil}
}
...
func (s *stateDB) GetMemoryFootprint() *common.MemoryFootprint {
	...
	mf.AddChild("state", s.database.GetMemoryFootprint())
...
func (s *stateDB) GetArchiveStateDB(block uint64) (NonCommittableState, error) {
	archiveState, err := s.database.GetArchiveState(block)
	if err != nil {
		return nil, err
	}
...
func (s *stateDB) GetArchiveBlockHeight() (uint64, bool, error) {
	return s.database.GetArchiveBlockHeight()
}
...
func (s *stateDB) resetState(state database.State) {
	s.resetBlockContext()
	s.storedDataCache.Clear()
...
func (l *bulkLoad) apply() {
	...
	err := l.db.Apply(l.block, l.update)
...
var nonCommittableStateDbPool = sync.Pool{
	New: func() any {
		...
		return createStateDBWith(nil, nil, nonCommittableStoredDataCacheSize)
	},
}
...
func (db *nonCommittableStateDB) Copy() NonCommittableState {

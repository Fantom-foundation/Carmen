...
func newGoMemoryS4State(params state.Parameters) (state.State, error) {
	state, err := mpt.OpenGoMemoryState(filepath.Join(params.Directory, "live"), mpt.S4LiveConfig, mpt.TrieConfig{CacheCapacity: mptStateCapacity(params.LiveCache)})
	if err != nil {
		return nil, err
	}
...
func newGoFileS4State(params state.Parameters) (state.State, error) {
	state, err := mpt.OpenGoFileState(filepath.Join(params.Directory, "live"), mpt.S4LiveConfig, mpt.TrieConfig{CacheCapacity: mptStateCapacity(params.LiveCache)})
	if err != nil {
		return nil, err
	}
	return newS4State(params, state)

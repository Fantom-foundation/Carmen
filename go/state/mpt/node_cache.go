...
func (c *nodeCache) Get(r *NodeReference) (*shared.Shared[Node], bool) {
	...
	// any time the cache may chose to evict an element and replace it another.
	// To do so, the owner at the corresponding position is simply updated. To
	// allow node references to identify situations in which the referenced
	// node got evicted, an additional tag is stored. This tag is incremented
	// every time an owner is recycled, allowing references to identify
	// modifications.

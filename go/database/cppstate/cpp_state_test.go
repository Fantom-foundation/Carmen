...
func TestAccountsAreInitiallyUnknown(t *testing.T) {
	runForEachCppConfig(t, func(t *testing.T, db database.Database) {
		account_state, _ := db.Exists(address1)
...
func TestAccountsCanBeCreated(t *testing.T) {
	runForEachCppConfig(t, func(t *testing.T, db database.Database) {
		db.Apply(1, common.Update{CreatedAccounts: []common.Address{address1}})
		account_state, _ := db.Exists(address1)
...
func TestAccountsCanBeDeleted(t *testing.T) {
	runForEachCppConfig(t, func(t *testing.T, db database.Database) {
		db.Apply(1, common.Update{CreatedAccounts: []common.Address{address1}})
		db.Apply(2, common.Update{DeletedAccounts: []common.Address{address1}})
		account_state, _ := db.Exists(address1)
		if account_state != false {
...
func TestReadUninitializedBalance(t *testing.T) {
	runForEachCppConfig(t, func(t *testing.T, db database.Database) {
		balance, err := db.GetBalance(address1)
		if err != nil {
...
func TestReadUninitializedNonce(t *testing.T) {
	runForEachCppConfig(t, func(t *testing.T, db database.Database) {
		nonce, err := db.GetNonce(address1)
		if err != nil {
...
func TestReadUninitializedSlot(t *testing.T) {
	runForEachCppConfig(t, func(t *testing.T, db database.Database) {
		value, err := db.GetStorage(address1, key1)
		if err != nil {
...
func TestSetAndGetCode(t *testing.T) {
	runForEachCppConfig(t, func(t *testing.T, db database.Database) {
		for _, code := range getTestCodes() {
			...
			size, err := db.GetCodeSize(address1)
			if err != nil {
...
func TestSetAndGetCodeHash(t *testing.T) {
	runForEachCppConfig(t, func(t *testing.T, db database.Database) {
		for _, code := range getTestCodes() {
			...
			hash, err := db.GetCodeHash(address1)
			if err != nil {
				t.Fatalf("Error fetching code: %v", err)
			}
...
func runForEachCppConfig(t *testing.T, test func(*testing.T, database.Database)) {

...
type CppState struct {
	// A pointer to an owned C++ object containing the actual database information.
	db unsafe.Pointer
	// cache of contract codes
	codeCache *common.LruCache[common.Address, []byte]
}

func newState(impl C.enum_StateImpl, params database.Parameters) (database.Database, error) {
	...
	case database.ArchiveType(""), database.NoArchive:
		archive = 0
	case database.LevelDbArchive:
		archive = 1
	case database.SqliteArchive:
		archive = 2
	default:
		return nil, fmt.Errorf("%w: unsupported archive type %v", database.UnsupportedConfiguration, params.Archive)
	}
	...
	if st == unsafe.Pointer(nil) {
		return nil, fmt.Errorf("%w: failed to create C++ state instance for parameters %v", database.UnsupportedConfiguration, params)
	}

	return database.WrapIntoSyncedDatabase(&CppState{
		db:        st,
		codeCache: common.NewLruCache[common.Address, []byte](CodeCacheSize),
	}), nil
}

func newInMemoryState(params database.Parameters) (database.Database, error) {
	return newState(C.kState_Memory, params)
}

func newFileBasedState(params database.Parameters) (database.Database, error) {
	return newState(C.kState_File, params)
}

func newLevelDbBasedState(params database.Parameters) (database.Database, error) {
...
func (cs *CppState) Exists(address common.Address) (bool, error) {
	...
	C.Carmen_GetAccountState(cs.db, unsafe.Pointer(&address[0]), unsafe.Pointer(&res))
...
func (cs *CppState) GetBalance(address common.Address) (common.Balance, error) {
	...
	C.Carmen_GetBalance(cs.db, unsafe.Pointer(&address[0]), unsafe.Pointer(&balance[0]))
	return balance, nil
}
...
func (cs *CppState) GetNonce(address common.Address) (common.Nonce, error) {
	...
	C.Carmen_GetNonce(cs.db, unsafe.Pointer(&address[0]), unsafe.Pointer(&nonce[0]))
	return nonce, nil
}
...
func (cs *CppState) GetStorage(address common.Address, key common.Key) (common.Value, error) {
	...
	C.Carmen_GetStorageValue(cs.db, unsafe.Pointer(&address[0]), unsafe.Pointer(&key[0]), unsafe.Pointer(&value[0]))
	return value, nil
}
...
func (cs *CppState) GetCode(address common.Address) ([]byte, error) {
	...
	C.Carmen_GetCode(cs.db, unsafe.Pointer(&address[0]), unsafe.Pointer(&code[0]), &size)
	if size >= CodeMaxSize {
...
func (cs *CppState) GetCodeHash(address common.Address) (common.Hash, error) {
	...
	C.Carmen_GetCodeHash(cs.db, unsafe.Pointer(&address[0]), unsafe.Pointer(&hash[0]))
	return hash, nil
}

func (cs *CppState) GetCodeSize(address common.Address) (int, error) {
	var size C.uint32_t
	C.Carmen_GetCodeSize(cs.db, unsafe.Pointer(&address[0]), &size)
	return int(size), nil
}

func (cs *CppState) GetHash() (common.Hash, error) {
	var hash common.Hash
	C.Carmen_GetHash(cs.db, unsafe.Pointer(&hash[0]))
	return hash, nil
}
...
func (cs *CppState) Apply(block uint64, update common.Update) error {
	...
	C.Carmen_Apply(cs.db, C.uint64_t(block), dataPtr, C.uint64_t(len(data)))
...
func (cs *CppState) Flush() error {
	C.Carmen_Flush(cs.db)
	return nil
}

func (cs *CppState) Close() error {
	if cs.db != nil {
		C.Carmen_Close(cs.db)
		C.Carmen_ReleaseState(cs.db)
		cs.db = nil
	}
	return nil
}
...
func (cs *CppState) GetMemoryFootprint() *common.MemoryFootprint {
	...
	var buffer *C.char
	var size C.uint64_t
	C.Carmen_GetMemoryFootprint(cs.db, &buffer, &size)
	defer func() {
...
func (cs *CppState) GetArchiveState(block uint64) (database.State, error) {
	return &CppState{
		db:        C.Carmen_GetArchiveState(cs.db, C.uint64_t(block)),
		codeCache: common.NewLruCache[common.Address, []byte](CodeCacheSize),
	}, nil
}

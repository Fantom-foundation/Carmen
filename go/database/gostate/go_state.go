...
type LiveDB interface {
	...
	// Called after synching to a new state, requesting the schema to update cached
	// values or tables not covered by the snapshot synchronization.
	RunPostRestoreTasks() error
}

func newGoState(live LiveDB, archive archive.Archive, cleanup []func()) database.Database {
	...
	return database.WrapIntoSyncedDatabase(res)
}
...
func (s *GoState) GetArchiveState(block uint64) (as database.State, err error) {
	if s.archive == nil {
		return nil, database.NoArchiveError
	}
...
func (s *GoState) GetArchiveBlockHeight() (uint64, bool, error) {
	if s.archive == nil {
		return 0, false, database.NoArchiveError
	}
	lastBlock, empty, err := s.archive.GetBlockHeight()
	if err != nil {

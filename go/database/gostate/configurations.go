...
const defaultSchema = database.Schema(1)

const (
	VariantGoMemory         database.Variant = "go-memory"
	VariantGoFile           database.Variant = "go-file"
	VariantGoFileNoCache    database.Variant = "go-file-nocache"
	VariantGoLevelDb        database.Variant = "go-ldb"
	VariantGoLevelDbNoCache database.Variant = "go-ldb-nocache"
)

func init() {
	generallySupportedArchives := []database.ArchiveType{
		database.NoArchive,
	...
	for schema := database.Schema(1); schema <= database.Schema(5); schema++ {
	...
	mptSetups := []struct {
		...
		archive database.ArchiveType
	}{
		{4, database.S4Archive},
		{5, database.S5Archive},
	}
...
func newGoMemoryState(params database.Parameters) (database.Database, error) {
	...
	default:
		return nil, fmt.Errorf("%w: the go implementation only supports schemas 1-5, got %d", database.UnsupportedConfiguration, params.Schema)
	}
...
func newGoFileState(params database.Parameters) (database.Database, error) {
...
func newGoCachedFileState(params database.Parameters) (database.Database, error) {
	path, err := getLiveDbPath(params)
	if err != nil {
		return nil, err
	}
...
func newGoLeveLIndexAndStoreState(params database.Parameters) (database.Database, error) {
	if params.Schema == 0 {
		params.Schema = defaultSchema
	}
	...
	default:
		return nil, fmt.Errorf("%w: the go implementation only supports schemas 1,2,3, got %d", database.UnsupportedConfiguration, params.Schema)
	}
...
func newGoCachedLeveLIndexAndStoreState(params database.Parameters) (database.Database, error) {
	if params.Schema == 0 {
		params.Schema = defaultSchema
	}
	db, err := backend.OpenLevelDb(params.Directory+string(filepath.Separator)+"live", nil)
...
func getLiveDbPath(params database.Parameters) (string, error) {
	path := filepath.Join(params.Directory, "live")
	return path, os.MkdirAll(path, 0700)
}

func getArchivePath(params database.Parameters) (string, error) {
	path := filepath.Join(params.Directory, "archive")
	return path, os.MkdirAll(path, 0700)
}

func openArchive(params database.Parameters) (archive archive.Archive, cleanup func(), err error) {

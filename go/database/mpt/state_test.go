...
var stateFactories = map[string]func(string) (io.Closer, error){
	"memory": func(dir string) (io.Closer, error) {
		return OpenGoMemoryState(dir, S5LiveConfig, TrieConfig{CacheCapacity: 1024})
	},
	"file": func(dir string) (io.Closer, error) {
		return OpenGoFileState(dir, S5LiveConfig, TrieConfig{CacheCapacity: 1024})
	},
	"archive": func(dir string) (io.Closer, error) {
		return OpenArchiveTrie(dir, S5ArchiveConfig, TrieConfig{CacheCapacity: 1024})
	},
...
func TestState_RegularCloseResultsInCleanState(t *testing.T) {
	...
	state, err := OpenGoFileState(dir, S5LiveConfig, TrieConfig{CacheCapacity: 1024})
	if err != nil {
		t.Fatalf("failed to open test state: %v", err)
	}
	...
	for _, config := range allMptConfigs {
		for _, withHashing := range []bool{false, true} {
			...
			b.Run(fmt.Sprintf("%s/%s", config.Name, mode), func(b *testing.B) {
				state, err := OpenGoMemoryState(b.TempDir(), config, TrieConfig{CacheCapacity: 1024})
				if err != nil {
					b.Fail()
				}
				defer state.Close()
...
func TestState_ForestErrorIsReportedInFlushAndClose(t *testing.T) {
	...
	dir := t.TempDir()
	state, err := OpenGoFileState(dir, S4LiveConfig, TrieConfig{CacheCapacity: 1024})
	if err != nil {
		t.Fatalf("failed to open test state: %v", err)
	}
...
func runFlushBenchmark(b *testing.B, config MptConfig, forceDirtyNodes bool) {
	numAccounts := 100_000
	state, err := OpenGoFileState(b.TempDir(), config, TrieConfig{CacheCapacity: numAccounts})
	if err != nil {

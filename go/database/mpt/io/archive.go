...
func ExportArchive(ctx context.Context, directory string, out io.Writer) error {
	...
	archive, err := mpt.OpenArchiveTrie(directory, info.Config, mpt.TrieConfig{CacheCapacity: mpt.DefaultMptStateCapacity})
	if err != nil {
		return err
	}
...
func importArchive(liveDbDir, archiveDbDir string, in io.Reader) (err error) {
	...
	live, err := mpt.OpenGoFileState(liveDbDir, mpt.S5LiveConfig, mpt.TrieConfig{CacheCapacity: mpt.DefaultMptStateCapacity})
	if err != nil {
	...
	archive, err := mpt.OpenArchiveTrie(archiveDbDir, mpt.S5ArchiveConfig, mpt.TrieConfig{CacheCapacity: mpt.DefaultMptStateCapacity})
	if err != nil {

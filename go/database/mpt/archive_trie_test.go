...
func TestArchiveTrie_OpenAndClose(t *testing.T) {
	for _, config := range allMptConfigs {
		t.Run(config.Name, func(t *testing.T) {
			archive, err := OpenArchiveTrie(t.TempDir(), config, TrieConfig{CacheCapacity: 1024})
			if err != nil {
...
func TestArchiveTrie_CanOnlyBeOpenedOnce(t *testing.T) {
	...
	if _, err := OpenArchiveTrie(dir, S5ArchiveConfig, TrieConfig{CacheCapacity: 1024}); err == nil {
...
func TestArchiveTrie_Open_Fails_Wrong_Roots(t *testing.T) {
	...
	if archive, err := OpenArchiveTrie(dir, S5ArchiveConfig, TrieConfig{CacheCapacity: 1024}); err == nil {
		_ = archive.Close()
...
func TestArchiveTrie_CanProcessPrecomputedHashes(t *testing.T) {
	for _, config := range allMptConfigs {
		...
		t.Run(config.Name, func(t *testing.T) {
			live, err := OpenGoMemoryState(t.TempDir(), config, TrieConfig{CacheCapacity: 1024})
			if err != nil {
				t.Fatalf("failed to open live trie: %v", err)
			}

			archiveDir := t.TempDir()
			archive, err := OpenArchiveTrie(archiveDir, config, TrieConfig{CacheCapacity: 1024})
			if err != nil {
				t.Fatalf("failed to open empty archive: %v", err)
			}
...
func TestArchiveTrie_VerificationOfFreshArchivePasses(t *testing.T) {
	for _, config := range allMptConfigs {
		t.Run(config.Name, func(t *testing.T) {
			dir := t.TempDir()
			archive, err := OpenArchiveTrie(dir, config, TrieConfig{CacheCapacity: 1024})
			if err != nil {
...
func TestArchiveTrie_Add_UpdateFailsHashing(t *testing.T) {
	for _, config := range allMptConfigs {
		t.Run(config.Name, func(t *testing.T) {
			...
			archive, err := OpenArchiveTrie(dir, config, TrieConfig{CacheCapacity: 0})
			if err != nil {
				t.Fatalf("failed to create empty archive, err %v", err)
			}
...
func TestArchive_RootsGrowSubLinearly(t *testing.T) {
	for _, config := range allMptConfigs {
		t.Run(config.Name, func(t *testing.T) {
			archive, err := OpenArchiveTrie(t.TempDir(), config, TrieConfig{CacheCapacity: 0})
			if err != nil {
				t.Fatalf("failed to create empty archive, err %v", err)
			}
			defer func() {
...
func TestArchiveTrie_QueryLoadTest(t *testing.T) {
	...
	archive, err := OpenArchiveTrie(t.TempDir(), S5ArchiveConfig, TrieConfig{CacheCapacity: 30_000})
	if err != nil {
...
func TestArchiveTrie_FailingGetterOperation_InvalidatesArchive(t *testing.T) {
	...
	for i := 0; i < len(archiveGetters); i++ {
		...
		t.Run(fmt.Sprintf("rotation_%d", i), func(t *testing.T) {
			...
			archive, err := OpenArchiveTrie(t.TempDir(), S4ArchiveConfig, TrieConfig{CacheCapacity: 1000})
			if err != nil {
...
func BenchmarkArchiveFlush_Roots(b *testing.B) {
	archive, err := OpenArchiveTrie(b.TempDir(), S5ArchiveConfig, TrieConfig{CacheCapacity: 1000})
	if err != nil {

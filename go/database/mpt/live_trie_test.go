...
func TestLiveTrie_EmptyTrieIsConsistent(t *testing.T) {
	for _, config := range allMptConfigs {
		t.Run(config.Name, func(t *testing.T) {
			trie, err := OpenInMemoryLiveTrie(t.TempDir(), config, TrieConfig{CacheCapacity: 1024})
			if err != nil {
...
func TestLiveTrie_Cannot_Open_Memory(t *testing.T) {
	for _, config := range allMptConfigs {
		t.Run(config.Name, func(t *testing.T) {
			...
			if _, err := OpenInMemoryLiveTrie(dir, config, TrieConfig{CacheCapacity: 1024}); err == nil {
...
func TestLiveTrie_Cannot_Open_File(t *testing.T) {
	for _, config := range allMptConfigs {
		t.Run(config.Name, func(t *testing.T) {
			...
			if _, err := OpenFileLiveTrie(dir, config, TrieConfig{CacheCapacity: 1024}); err == nil {
				t.Errorf("opening trie should fail")
			}
...
func TestLiveTrie_Fail_Read_Data(t *testing.T) {
	for _, config := range allMptConfigs {
		t.Run(config.Name, func(t *testing.T) {
			...
			mpt, err := OpenFileLiveTrie(dir, config, TrieConfig{CacheCapacity: 1024})
			if err != nil {
...
func TestLiveTrie_SameContentProducesSameHash(t *testing.T) {
	for _, config := range allMptConfigs {
		t.Run(config.Name, func(t *testing.T) {
			trie1, err := OpenInMemoryLiveTrie(t.TempDir(), config, TrieConfig{CacheCapacity: 1024})
			if err != nil {
				t.Fatalf("failed to open trie: %v", err)
			}
			trie2, err := OpenInMemoryLiveTrie(t.TempDir(), config, TrieConfig{CacheCapacity: 1024})
			if err != nil {
				t.Fatalf("failed to open trie: %v", err)
			}
...
func TestLiveTrie_InsertLotsOfData(t *testing.T) {
	for _, config := range allMptConfigs {
		...
		t.Run(config.Name, func(t *testing.T) {
			...
			trie, err := OpenInMemoryLiveTrie(t.TempDir(), config, TrieConfig{CacheCapacity: 1024 * 1024})
			if err != nil {
				t.Fatalf("failed to open trie: %v", err)
			}
			defer trie.Close()
...
func TestLiveTrie_VerificationOfFreshArchivePasses(t *testing.T) {
	for _, config := range allMptConfigs {
		t.Run(config.Name, func(t *testing.T) {
			dir := t.TempDir()
			trie, err := OpenFileLiveTrie(dir, config, TrieConfig{CacheCapacity: 1024})
			if err != nil {
...
func BenchmarkValueInsertionInMemoryTrie(b *testing.B) {
	for _, config := range allMptConfigs {
		b.Run(config.Name, func(b *testing.B) {
			b.StopTimer()
			trie, err := OpenInMemoryLiveTrie(b.TempDir(), config, TrieConfig{CacheCapacity: 1024})
			if err != nil {
...
func BenchmarkValueInsertionInFileTrie(b *testing.B) {
	for _, config := range allMptConfigs {
		b.Run(config.Name, func(b *testing.B) {
			b.StopTimer()
			trie, err := OpenFileLiveTrie(b.TempDir(), config, TrieConfig{CacheCapacity: 1024})
			if err != nil {
				b.Fatalf("failed to open trie: %v", err)
			}
			defer trie.Close()

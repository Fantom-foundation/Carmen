...
// Each operation is applied to the system under fuzzing.
type Operation[T any] interface {
	// Apply is executed by the fuzzer for each operation to
	// perform required changes to the system under test.
	// The context passed to this method carries the system under the test
	// and is state-full carrying also all state data required to move
...
type Campaign[T any] interface {
	...
	// Each sequence is used as a seed for the fuzzer.
	// This method is called only once before the campaign starts.
	Init() []OperationSequence[T]

	// CreateContext should create an object that holds the system under the fuzzing
	// plus all state information that are needed to be hold between each steps of the campaign.
	...
	// Deserialize gets byte array generated by the fuzzer, which represent a set of operations.
	// These operations are decoded and returned to the fuzzer. The fuzzer loops on the operations
